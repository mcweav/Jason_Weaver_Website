...
const ARROW_CLASSES =
  "relative flex items-center justify-center rounded-full border-[1.5px] border-ink/10 bg-surface text-ink/55 cursor-pointer shrink-0 z-30 outline-none shadow-[0_4px_20px_rgba(0,0,0,0.12)] hover:border-ink/25 hover:text-ink/80 active:opacity-70 transition-colors duration-300";
  ...
  return (
    <section className="relative z-20 flex w-full flex-col items-center px-4 py-4 md:px-8 lg:py-8">
      <div className="flex w-full max-w-[90rem] items-center justify-center">
        <div
          ref={containerRef}
          className="fan-layout relative flex h-[22rem] w-full max-w-[80rem] items-center justify-center sm:h-[26rem] md:h-[28rem] lg:h-[34rem] xl:h-[38rem]"
        >
          {cards.map((card, index) => {
            const image = (
              <div className="relative h-full w-full overflow-hidden rounded-2xl border border-ink/10 shadow-2xl">
                <img
                  src={card.imgUrl}
                  loading="lazy"
            ...
            const cardClasses =
              "fan-card absolute inset-0 m-auto aspect-4/7 w-24 sm:w-32 md:w-36 lg:w-44 xl:w-52";
            return card.linkUrl ? (
              <a
                key={index}
                href={card.linkUrl}
            ...
            {cards.map((_, i) => (
              <span
                ...
                className={`h-2 w-2 rounded-full transition-all duration-300 ${
                  i === centerIndex ? "scale-[1.3] bg-ink/70" : "bg-ink/15"
                }`}
              />
            ))}
          </div>
